-- Targeted UUID Fix Based on Actual Schema
-- This addresses the specific constraints found in your debug output

BEGIN;

-- STEP 1: Drop ALL the foreign key constraints we found
...
-- REMOVED: ALTER TABLE schedules ALTER COLUMN project_id TYPE TEXT; (This column doesn't exist)
...
-- Schedules -> custom_users and tasks
ALTER TABLE schedules 
ADD CONSTRAINT schedules_user_id_fkey 
FOREIGN KEY (user_id) REFERENCES custom_users(id) ON DELETE CASCADE;

-- REMOVED: project_id foreign key constraint for schedules since the column doesn't exist
ALTER TABLE schedules 
...
-- STEP 5: Add UUID format validation constraints (CORRECTED)
...
-- REMOVED: CREATE INDEX IF NOT EXISTS idx_schedules_project_id_text ON schedules(project_id);
